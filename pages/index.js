...
export default function Home() {
    ...
    useEffect(() => {
        ...
        const updateSoonLogic = () => {
            ...
            if (!next) next = gameList[4]; // Default to L7 if late night
        ...
        <div style={{ backgroundColor: '#fff', minHeight: '100vh', fontFamily: '"Arial Narrow", sans-serif', textAlign: 'center', margin: 0, padding: 0, overflowX: 'hidden' }}>
            ...
            <div style={{ padding: '20px 10px' }}>
                ...
                <div style={{ marginTop: '15px' }}>
                    <h2 className="blink-soon" style={{ color: 'red', fontSize: '45px', fontWeight: '900', margin: 0 }}>{latestGame ? latestGame.name : nextGameSoon}</h2>
                    ...
                    <table style={{ width: '100%', borderCollapse: 'collapse', backgroundColor: 'white', color: '#000', fontSize: '12px' }}>
                        <thead>
                            <tr style={{ backgroundColor: '#800000', color: 'white' }}>
                                ...
                                {gameList.map(g => <th key={g.id} style={{ border: '1.5px solid black', padding: '2px' }}>{g.name.split(' ')[0]}</th>)}
                            </tr>
                        </thead>
                        <tbody>
                            {[...Array(30)].map((_, i) => {
                                ...
                                return (
                                    <tr key={day} style={{ height: '32px' }}>
                                        ...
                                        {gameList.map(g => <td key={g.id} style={{ border: '1.5px solid black', fontWeight: 'bold', fontSize: '17px' }}>{getAprilVal(day, g.id)}</td>)}
                                    </tr>
                                );
                            })}
                        </tbody>
